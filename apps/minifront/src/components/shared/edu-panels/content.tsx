...
export const eduPanelContent: Record<EduPanel, string> = {
  [EduPanel.ASSETS]:
    'Your balances are shielded, and are known only to you. They are not visible on chain. Each Shieldd wallet controls many numbered accounts, each with its own balance. Account information is never revealed on-chain.',
  ...
  [EduPanel.SHIELDED_TRANSACTION]:
    'Shieldd transactions are shielded and don’t reveal any information about the sender, receiver, or amount.  Use the toggle to see what information is revealed on-chain.',
  [EduPanel.SENDING_FUNDS]:
    'Shieldd transactions include a shielded memo only visible to the sender and receiver.  The sender’s address is included in the memo so the receiver can identify the payment.',
  [EduPanel.RECEIVING_FUNDS]:
    'Every Shieldd account has a stable default address and many one-time IBC deposit addresses.  All addresses for the same account deposit to the same pool of funds.  Use a freshly generated IBC deposit address to preserve privacy when sending funds from a transparent chain to Shieldd.',
