...
import { SwapRecord } from '@mizufinance/protobuf/shieldd/view/v1/view_pb';
...
const fetchMetadataForSwap = async (swap: SwapRecord): Promise<UnclaimedSwapsWithMetadata> => {
  ...
  const [{ denomMetadata: asset1Metadata }, { denomMetadata: asset2Metadata }] = await Promise.all([
    shieldd.service(ViewService).assetMetadataById({ assetId: assetId1 }),
    shieldd.service(ViewService).assetMetadataById({ assetId: assetId2 }),
  ]);
...
export const fetchUnclaimedSwaps = async (): Promise<UnclaimedSwapsWithMetadata[]> => {
  const responses = await Array.fromAsync(shieldd.service(ViewService).unclaimedSwaps({}));
