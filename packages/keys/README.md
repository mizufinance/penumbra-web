...
```ts
const fetchConvertPk = await fetch(import.meta.resolve('@mizufinance/keys/convert_pk.bin'));
...
async function getActionProvingKey({ action }: Action) {
  ...
  if (keyName) {
    const fetchKey = await fetch(import.meta.resolve(keyName, '@mizufinance/keys'));
